...
For the hands-on part of this guide, you will build upon [this tutorial with the emojivoto application](../../quick-start/demo-node/), which is written in Go. 
...
3. Ensure that your development environment is configured to support the automatic reloading of the service when your source code changes.
 In the example, the Go applicationapplication source code is being monitored for changes, and the application is rebuilt with Air's live-reloading utility.
...
If you followed the [emojivoto application tutorial](../../quick-start/demo-node/) referenced at the beginning of this guide, you will have already downloaded the emojivoto development container. Now, clone the emojivoto code repo and run the container with updated configuration to expose the applicaiton’s ports locally and volume mount your local copy of the application source code into the container:
...
2. Extract the KUBECONFIG from your dev container from the [emojivoto application tutorial](../../quick-start/demo-node/) and then connect your container to the remote cluster with Telepresence:
	```
...
6. Run the following three commands to aenerate a link to share with your teammates:
	```
